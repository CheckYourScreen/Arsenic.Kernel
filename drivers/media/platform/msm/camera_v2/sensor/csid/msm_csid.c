...
static int msm_csid_init(struct csid_device *csid_dev, uint32_t *csid_version)
{
	...
	if (csid_dev->ctrl_reg->csid_reg.csid_version == CSID_VERSION_V20)
		cam_vreg = csid_8960_vreg_info;
	else
		cam_vreg = csid_vreg_info;

	if (csid_dev->ctrl_reg->csid_reg.csid_version < CSID_VERSION_V30) {
		...
		if (csid_dev->ctrl_reg->csid_reg.csid_version ==
						CSID_VERSION_V20) {
	...
	} else if (csid_dev->ctrl_reg->csid_reg.csid_version >=
						CSID_VERSION_V30) {
		rc = msm_camera_config_vreg(&csid_dev->pdev->dev,
			csid_vreg_info, ARRAY_SIZE(csid_vreg_info),
			NULL, 0, &csid_dev->csi_vdd, 1);
